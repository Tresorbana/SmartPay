...
// MongoDB Atlas connection - use .env file for security
...
async function getSystemStats() {
    try {
        ...
        const activeWallets = await walletCollection.countDocuments();
        ...
        stats.forEach(s => {
            if (s._id === 'TOPUP') {
                result.topup = { total: s.totalAmount, count: s.count };
            } else if (s._id === 'PAYMENT') {
                result.payment = { total: s.totalAmount, count: s.count };
            }
        });
...
// Get all transactions for a global history view
            ...
            const defaultProducts = [
                {
                    ...
                    price: 85,
                    active: true,
                    description: "Standard synthetic oil change and filter replacement",
                    createdAt: new Date()
                },
                {
                ...
                },
                {
                    ...
                    price: 245,
                    active: true,
                    description: "Front brake pad replacement and inspection",
                    createdAt: new Date()
                },
                {
                    ...
                    price: 45,
                    active: true,
                    description: "24-hour sedan rental",
                    createdAt: new Date()
                }
            ];

            await productsCollection.insertMany(defaultProducts);
            console.log("✓ Car Dealership products seeded");
        }
    } catch (error) {
